    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %s simp %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s simp %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64_t& nMinWeight, uint64_t& nMaxWeight, uint64_t& nWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        if (GetTime() < 1480335300) // Human time (GMT): Mon, 28 Nov 2016 12:15:00 GMT
        ...
        //  Add accumulated weight to .9 so coins are eligible to stake sooner. Boost coin weight 1000x
            ...
            if (GetTime() < 1481372100) nWeightFactor = 1000;             // Human time (GMT): Sat, 10 Dec 2016 12:15:00 GMT -- Boost 1000x
            else if (GetTime() < 1481457600) nWeightFactor = 100;         // Human time (GMT): Sun, 11 Dec 2016 12:00:00 GMT -- Slowly Phase Out
            else if (GetTime() >= 1481457600) nWeightFactor = 10;         // Human time (GMT): Sun, 11 Dec 2016 12:00:00 GMT -- Phase out to 10x weight
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s simp %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s simp %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), pcoin->GetHash().ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
