...
const string strMessageMagic = "simplecash Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int blkheight)
{
    ...
    if (blkheight <= LAST_POS_BLOCK_GERMINATION)
        nCoinReward = COIN_YEAR_REWARD;
    else
    if (blkheight <= LAST_POS_BLOCK_GROWTH)
        nCoinReward = COIN_DAY_REWARD;
    else
    if (blkheight < (BLOCK_YEAR * 5))
    {
        int coinyear = 1 + (blkheight / BLOCK_YEAR);
        nCoinReward = MAX_COIN_YEAR_REWARD / coinyear;
    }
    else
        nCoinReward = COIN_YEAR_REWARD;
    ...
    int64_t nSubsidy;
    if (pindexBest->nTime > 1480420800) // Human time (GMT): Tue, 29 Nov 2016 12:00:00 GMT
    {
        if (nCoinAge > COIN && pindexBest->nTime > 1480431600) // Human time (GMT): Tue, 29 Nov 2016 15:00:00 GMT
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "simplecash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "simplecash is born- 28-11-2016";
        CTransaction txNew;
        txNew.nTime = 1480341166;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime = 1480341166;
        block.nBits = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2973458 : 2973458;
        ...
        assert(block.hashMerkleRoot == uint256("0x2fdadeaee5926b5a7d99a825314c80f35c336ded553d9b17f864061fb466b93e"));
...
unsigned char pchMessageStart[4] = { 0x4f, 0x05, 0x02, 0x03 };
